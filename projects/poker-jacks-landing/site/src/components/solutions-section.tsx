    ...
    >
      ...
      <ol className="mt-10">
        {t.solutions.items.map((item, i) => (
          ...
          >
            <span
              className="inline-flex h-6 w-9 items-center justify-center justify-self-start rounded-md bg-[#3B4BC8] font-heading text-sm font-bold text-white sm:mt-0.5"
...
/* Mini-visual de Feria: tiles de catálogo en teal/coral/índigo (flat, sin gradientes) */
function FeriaTiles() {
  const tiles = [
    "bg-[#1F6E7E]",
    "bg-[#FDEEE3]",
  ...
  return (
    <div className="flex gap-1.5" aria-hidden="true">
      {tiles.map((bg, i) => (
        <span key={i} className={`size-7 rounded-md ${bg}`} />
      ))}
    </div>
  );
}

/* Mini-visual de Rumbo: ruta punteada teal con nodos coral */
function RumboRoute() {
  return (
    <svg
      width="188"
    ...
    >
      <path
        d="M6 22C40 22 52 6 94 6s54 16 88 16"
        stroke="#1F6E7E"
        strokeOpacity="0.45"
        strokeWidth="2"
        strokeDasharray="5 5"
        strokeLinecap="round"
      />
      <circle cx="6" cy="22" r="5" fill="#F97316" />
      <circle cx="94" cy="6" r="5" fill="#F97316" />
      <circle cx="182" cy="22" r="5" fill="#F97316" />
    </svg>
  );
}
  ...
  return (
    ...
    >
      ...
      <div className="mt-8 grid gap-10 sm:grid-cols-2">
        {t.early.products.map((product, i) => (
          <div key={product.name}>
            <div className="flex h-8 items-center">
              {i === 0 ? <FeriaTiles /> : <RumboRoute />}
            </div>
